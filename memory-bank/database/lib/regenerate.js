...
export async function regenerateSessionCache(outputPath) {
  ...
  const sessionFile = latestSession
    ? `sessions/${latestSession.date}-${latestSession.session_period}.md`
    : 'sessions/latest.md';
...
export async function regenerateSessionFile(sessionsDir) {
  ...
  try {
    sessions = await sqlite.queryAll(
      `SELECT id, date, session_period, focus_task, status, content, start_time, end_time
       FROM sessions
       ORDER BY date DESC, created_at DESC, id DESC`
    );
  } catch (err) {
    ...
    sessions = await sqlite.queryAll(
      `SELECT id, date, session_period, focus_task, status, content
       FROM sessions
  ...
  for (const session of sessions) {
    const fileName = `${session.date}-${session.session_period}.md`;
    const filePath = join(sessionsDir, fileName);

    let md = `# Session: ${session.date} ${session.session_period.charAt(0).toUpperCase() + session.session_period.slice(1)}\n\n`;
